...
void call_video_play(AVFrame *frame) {
    ...
    LOGE("绘制 宽%d,高%d, format:%d", frame->width, frame->height,frame->format);
...
//开启播放
        ...
        if (ret == 0) {
            ...
            while (isPlay) {
                if (ffmpegVideo->queue.empty() && ffmpegMusic->queue.empty()) {
                 ...
                 LOGE("等待播放完成,video empty:%d, audio empty:%v",ffmpegVideo->queue.empty(),ffmpegMusic->queue.empty());
                av_usleep(500000);
            }
        }
    }
    ...
    LOGE("播放完成退出");
    pthread_exit(0); //只是推出当前线程，一定要用调用join来释放资源
}

extern "C"
JNIEXPORT void JNICALL
Java_com_example_alexnet_ffmpeg_video_videoplayer_play
  (JNIEnv *env, jobject instance, jstring inputPath_){
    if(isPlay){
        LOGE("正在播放重复点击无效");
        return;
    }
  ...
  inputPath = env->GetStringUTFChars(inputPath_, 0);
    ...
    //pthread_join(p_tid, 0); 不要卡在主线程

}

extern "C"
JNIEXPORT void JNICALL
Java_com_example_alexnet_ffmpeg_video_videoplayer_display(JNIEnv *env, jobject instance, jobject surface) {
    //得到界面
    if (window) {
...
extern "C"
JNIEXPORT void JNICALL
Java_com_example_alexnet_ffmpeg_video_videoplayer_stop(JNIEnv *env, jobject instance) {
    //释放资源
    LOGE("STOP 停止播放");
    if (ffmpegMusic== nullptr||ffmpegVideo== nullptr||isPlay==0){
        LOGE("在开始播放前点击stop无效");
        return;
    }
    if (isPlay) {
...
extern "C"
JNIEXPORT void JNICALL
Java_com_example_alexnet_ffmpeg_video_videoplayer_pause(JNIEnv *env, jobject instance) {
    //点击暂停按钮
    LOGE("暂停播放push");
    if (ffmpegMusic== nullptr||ffmpegVideo== nullptr||isPlay==0){
        LOGE("播放停止pause 无效");
...
Java_com_example_alexnet_ffmpeg_video_videoplayer_getTotalTime(JNIEnv *env, jobject instance) {
...
Java_com_example_alexnet_ffmpeg_video_videoplayer_getCurrentPosition(JNIEnv *env, jobject instance) {
    //获取音频播放时间
    if(!isPlay){
        return -1;
    }
...
extern "C"
JNIEXPORT void JNICALL
Java_com_example_alexnet_ffmpeg_video_videoplayer_seekTo(JNIEnv *env, jobject instance, jint msec) {
seekTo(msec/1000);
} 

extern "C"
JNIEXPORT void JNICALL
Java_com_example_alexnet_ffmpeg_video_videoplayer_stepBack(JNIEnv *env, jobject instance) {

}

extern "C"
JNIEXPORT void JNICALL
Java_com_example_alexnet_ffmpeg_video_videoplayer_stepUp(JNIEnv *env, jobject instance) {
    //点击快进按钮
    seekTo(5);
}
