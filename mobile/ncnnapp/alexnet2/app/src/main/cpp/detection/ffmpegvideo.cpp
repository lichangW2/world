...
void *videoPlay(void *args){
    ...
    while (ffmpegVideo->isPlay) {
        ...
        sws_scale(ffmpegVideo->swsContext,(const uint8_t *const *)frame->data,frame->linesize,0,
                  ...
                  rgb_frame->linesize);
        LOGE("原始frame 宽%d,高%d，format:%d",frame->width,frame->height,frame->format);
        LOGE("rgb格式 宽%d,高%d,channels:%d",rgb_frame->width,rgb_frame->height,rgb_frame->channels);
...
int FFmpegVideo::get(AVPacket *avPacket) {
    ...
    while (isPlay){
        if(!queue.empty()&&isPause){
            ...
            av_free(packet2);
            break;
        } else if((!queue.empty()&&!isPause)||(queue.empty()&&!isEnd)){
            LOGE("video 执行wait");
            pthread_cond_wait(&cond,&mutex);
        } else{
            LOGE("video 队列为空，未获取到帧");
            //av_usleep(1000000);
            isPlay=0;
            break;
        }
...
void FFmpegVideo::stop() {
    ...
    LOGE("VIDEO join pass,pid:%d",playId);
    if (this->codec) {
